...
var el = 0;
export function get(num: number){
    el ++;
    return DATA.slice(DATA.length - num * el - 1, DATA.length - num * (el - 1))
}


export function getSliceOfArrayByTimestamp(array: { time: number, open: number, close: number, low: number, high: number }[], from: number, to: number) {
